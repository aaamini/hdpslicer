...
class HDP_gibbs_sampling:
    ...
    def get_doc_topic_freq(self):
        return self.n_m_z.copy()

def dai_warpper(docs, max_iter = 2):
    import numpy as np
    
    docs = [[word-1 for word in doc] for doc in docs] # map to zero index ids
    vocab = np.unique(np.array(docs))
    nwords = vocab.size
    
    HDP = HDP_gibbs_sampling(K0=20, alpha=0.5, beta=0.5, gamma=2, docs=docs, V=nwords) # initialize the HDP
    res = []
    for i in range(max_iter):
        HDP.inference(i)
        res.append(HDP.get_doc_topic_freq())
...
#     max_iter = 3 # number of iterations for getting converged
