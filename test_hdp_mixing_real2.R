...
J <- 894
fname <- paste("J", J, "_paper_labels", sep="")
...
#zh <- lapply(1:ITRmax,  function(itr) lapply(1:J, function(j) res$Zb[[itr]][j,1:n[j]]))
...
length(table(zh_flattend[ITRmax]))

zh_final <- zh[[ITRmax]]
save("zh_final", "y", "z", file="zh.RData")

export(p, file=paste("nmi", fname, ".pdf"))
export(p, file=paste("nmi", fname, ".png"))

# save(out, file="real_niloo4.RData")
